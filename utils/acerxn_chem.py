...
class Molecule:
    ...
    def set_multiplicity(self, multiplicity):
        ...
        if chg is None:
            ...
            if num_electron % 2 != (multiplicity + 1) % 2:
                print(
                    f"There are {num_electron} electrons, therefore spin with {multiplicity} is impossible!"
                )
            else:
    ...
    def get_molecule_id(self):
        ...
        molecule_id = int((self.get_chg() + 5) * np.sum(atom_id_list))
        return molecule_id
    ...
    def get_matrix(self, type_of_matrix="bo"):
        ...
        if type_of_matrix == "bo":
            ...
            coulomb_matrix = np.matmul(
                np.matmul(diagonal_matrix, new_adj_matrix), diagonal_matrix
            )
    ...
    def check_matrix(self, type_of_matrix="bo"):
        ...
        if type_of_matrix == "bo":
            if self.get_bo_matrix() is None:
                print(
                    "bo matrix is not prepared!!! It is necessary to define bo matrix"
                )
                return False
            return True
    ...
    def get_num_of_lone_pair_list(self):
        ...
        if chg_list is None:
            print(
                "chg is not prepared!!! It is necessary to obtain number of lone pairs!"
            )
            return None
        total_bond_order_list = self.get_total_bond_order_list()
        if total_bond_order_list is None:
            print(
                "bo matrix is not prepared!!! It is necessary to define bo matrix to obtain number of lone pairs!"
            )
            return None
    ...
    def get_num_of_pi_bond_list(self):
        ...
        if bo_matrix is None:
            print(
                "bo matrix is not prepared!!! It is necessary to define bo matrix to obtain the number of pi bonds!"
            )
            return None
    ...
    def get_octet_valence_list(self):
        ...
        if atom_feature is not None and "octet valence" in atom_feature:
            ...
            for p in range(1, 5):
                ...
                octet_valence_list = np.where(
                    period_list == p, octet_valence, octet_valence_list
                )
    ...
    def get_over_octet_indices(self):
        ...
        over_octet_indices += np.where(total_valence_list > octet_candidates)[
            0
        ].tolist()
    ...
    def get_bond_list(self, contain_bond_order=True):
        ...
        if contain_bond_order:
            bo = self.get_matrix("bo")
            if check_matrix is None and bo is None:
                ...
                # print("We will automatically give bond order as 1!")
    ...
    def get_trunc_bond_list(self, trunc_atom_indices, contain_bond_order=True):
        ...
        for bond in bond_list:
            ...
            atom2 = bond[1]
            if (
                trunc_atom_indices.count(atom1) == 1
                and trunc_atom_indices.count(atom2) == 1
            ):
                if atom1 < atom2:
                    if contain_bond_order:
                        bond_order = bond[2]
    ...
    def get_sssr(self):
        ...
        # try:
        rd_mol = self.get_rd_mol()
        # a = Chem.SanitizeMol(rd_mol)
        sssr_vectors = Chem.GetSymmSSSR(rd_mol)
        for i in range(len(sssr_vectors)):
            sssr.append(list(sssr_vectors[i]))
        # except:
        #    sssr = []
        return sssr
    ...
    def make_3d_coordinates(self, num_conformer=1, library="rdkit", max_iteration=1000):
        """
        ...
        if library == "rdkit":
            ...
            # conformer_id_list = AllChem.EmbedMultipleConfs(mol, numConfs=num_sample)
            conformer_energy_list = dict()
            for conformer_id in conformer_id_list:
                converged = not AllChem.UFFOptimizeMolecule(
                    mol, confId=conformer_id, maxIters=max_iteration
                )
                conformer_energy_list[conformer_id] = AllChem.UFFGetMoleculeForceField(
                    mol, confId=conformer_id
                ).CalcEnergy()
            if len(conformer_energy_list) == 0:
                print("No conformer is generated !!!")
                print(self.get_smiles("ace"))
                return []
            conformers = mol.GetConformers()
            n = len(z_list)
            for conformer_id in conformer_energy_list:
            ...
            """
            if self.smiles != None: # Using SMILES ruins permutation order!!!
                #### obabel method
                os.system('obabel -:"'+self.smiles+'" -O '+'tmp'+'.xyz --gen3d --nconf 50 --score energy')
                molecule = read('tmp.xyz', format ='xyz')
                os.system('rm tmp.xyz')
                coordinate_list = molecule.get_positions()
            """
            #### pybel method
            try:
            ...
            for i in range(num_conformer):
                coordinate_list = []
                pybel_mol.make3D()
                pybel_mol.localopt("uff", max_iteration)
                ...
                # Change coordinate list using uff opt
                if len(coordinate_list) > 0:
                    coordinates.append(np.array(coordinate_list))
        else:
        ...
        # Reupdate geometry for converged one ...
        for i in range(len(coordinates)):
            ...
            if use_ic_update:
                ...
                # ic.update_geometry(coordinate_list,q_updates)
    ...
    def write_input(
        ...
        if template_directory is None:
            print(
                "Warning: Generated input is for running DFT calculation with Gaussian!!!"
            )
    ...
    def write_uff_input(self, save_directory):
        ...
        if save_directory[-4:] != ".xyz":
            ...
            save_directory = os.path.join(save_directory, "tmp.com")
        input_file.write(
            "#N opt uff=qeq geom=connectivity iop(2/12=1, 4/120=100000)\n\nTitle\n\n"
        )
        input_file.write(str(chg) + " " + str(mult) + "\n")
        # All specified
        atom_list = self.atom_list
                    ...
                    for key_feature in molecule_atom_feature:
                        if key_feature in atom_feature:
                            atom_feature[key_feature] = np.concatenate(
                                (
                                    atom_feature[key_feature],
                                    molecule_atom_feature[key_feature],
                                )
                            )
                        else:
                            atom_feature[key_feature] = molecule_atom_feature[
                                key_feature
                            ]
    ...
    def get_smiles(self, method="none", find_stereocenter="N"):
        ...
        if self.smiles != None:
            ...
            for i in range(len(molecule_list)):
                ...
                if old_chg is None:
                    molecule.chg = int(
                        np.sum(fc_list[atom_indices_for_each_molecule[i]])
                    )
